...
int main()
{
  ...
  while (1)
  {
    ////////////////////////////////
    i = 0;
    memset(readdate, 0, sizeof(char) * 128); //char pointer 초기화
    int packetSize = parsePacket();
    if (packetSize)
    {
      while (available())
      {
        readdate[i] = (char)Lora_read();
      ...
      if (test_mode == 2)
      {
      //parse_date_NOTX(readdate);
      printf("%s\n", readdate);
      printf("%d\n",readRegister(REG_PKT_RSSI_VALUE));
      printf("%d\n",readRegister(REG_PKT_SNR_VALUE));
      }
      else{
      //parse_date(snd_thread, sock, readdate);
      printf("%s\n", readdate);     
      }
